pipeline{
    ...
    stages{
        stage("Build Docker image"){
         ...
         sh "scp -o StrictHostkeyChecking=no -r docker-files ${SERVER_IP}: /root/"
         sh "ssh -o StrictHostkeyChecking=no ${SERVER_IP} 'bash ~/docker-files/docker-script.sh'"
         sh "ssh -o StrictHostkeyChecking=no ${SERVER_IP} sudo docker build -t ${IMAGE_NAME} /root/docker-files/dockerfile" 
